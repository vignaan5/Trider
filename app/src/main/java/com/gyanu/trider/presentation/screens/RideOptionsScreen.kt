...
@Composable
fun RideOptionsScreen(vm:MapViewModel,navController: NavController){
    ...
    LaunchedEffect(true) {
        distanceScope.launch {
          ...
          val res = api.getDistanceMatrixDetails(origins = vm.mstate.pickupPinLoaction.latitude.toString()+","+vm.mstate.pickupPinLoaction.longitude.toString(), destinations = vm.mstate.dropPinLocation.latitude.toString()+","+vm.mstate.dropPinLocation.longitude.toString()).await()

                  if(res.status=="OK"){

                    distance =   res.rows[0].elements[0].distance.value.toInt()
                  }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Column(modifier = Modifier
            ...
            Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Bottom, modifier = Modifier.fillMaxWidth(1f) ) {
